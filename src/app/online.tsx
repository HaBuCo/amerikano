  ...
  return <SafeAreaView style={s.page}>
    <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
      ...
      {!room ? <>
        ...
        {room.members.map(m => <View key={m.id} style={s.member}><Text style={s.white}>{m.name}{m.id === room.you ? ' (sen)' : ''}{m.id === room.hostId ? ' ♛' : ''}</Text><Text style={s.status}>{!m.connected ? 'Bağlantı koptu' : m.ready ? '✓ Hazır' : 'Bekleniyor'}</Text></View>)}
...
const s = StyleSheet.create({
  ...
  back: { color: p.cream, fontSize: 15, paddingVertical: 10 }, eyebrow: { color: p.gold, letterSpacing: 3, fontSize: 11, marginTop: 20, fontWeight: '800' },
  ...
  member: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: p.line },
